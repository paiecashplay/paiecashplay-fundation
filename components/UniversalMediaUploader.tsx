...
import { Image as ImageIcon, Video, Trash2, Eye, User, Building, Users } from 'lucide-react';
...
export default function UniversalMediaUploader({ 
  ...
  const getIcon = () => {
    switch (entityType) {
      case 'player':
        return <User className="w-5 h-5 mr-2 text-[#4FBA73]" />;
      case 'club':
        return <Building className="w-5 h-5 mr-2 text-[#4FBA73]" />;
      case 'user':
        return <Users className="w-5 h-5 mr-2 text-[#4FBA73]" />;
    }
  };
  ...
  const getTitle = () => {
    ...
    switch (entityType) {
      case 'player':
        return 'Ajouter des médias du joueur';
      case 'club':
        return 'Ajouter des médias du club';
      case 'user':
        return 'Ajouter vos médias';
    }
  };
  ...
  const handleUpload = async (files: File[]) => {
    ...
    try {
      ...
      onMediaUpdate();
      toast.success('Upload réussi', 'Vos médias ont été ajoutés');
    } catch (error) {
  ...
  const deleteMedia = async (mediaId: string) => {
    ...
    try {
      const response = await fetch(`${getApiEndpoint()}?mediaId=${mediaId}`, {
        method: 'DELETE'
      });
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        {images.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <p>Aucune photo</p>
            {canEdit && (
              <p className="text-sm mt-2">Utilisez la zone d'upload ci-dessus pour ajouter des photos</p>
            )}
          </div>
        ) : (
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        {videos.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <p>Aucune vidéo</p>
            {canEdit && (
              <p className="text-sm mt-2">Utilisez la zone d'upload ci-dessus pour ajouter des vidéos</p>
            )}
          </div>
        ) : (
